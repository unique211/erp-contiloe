...
// for getting Monthy Cost location
	$this->db->select('sum(amount) as locationamount'); 
	$this->db->from('monthly_expense');
	$this->db->where("projectid",$project);
	$queryl11 = $this->db->get();
	if($queryl11->num_rows()>0){
		foreach($queryl11->result_array() as $locationmonth){
			$locationcost=$locationmonth['locationamount'];
		}
...
if($query5->num_rows() >0){
	foreach($query5->result_array() as $peopledata){
		...
		$rate=$peopledata['perdayrate'];
		$overtimerate=$peopledata['overtimerate'];
		if(	$overtime >0){
...
//RAVI
$day_totalunit=array();
$day_totalunit = $totalunit;
/*
for($i=1;$i<4;$i++){
	
	//$this->db->distinct('actualschedual.date');
	$this->db->select('sum(perdayrate + (extrahour*overtimerate)) as total'); 
	$this->db->from('peopleattdance');
	$this->db->join('actualschedual', 'actualschedual.id = peopleattdance.aid');
	$this->db->join('lis_artist', 'lis_artist.pid = peopleattdance.peopleid');
	$this->db->where('lis_artist.type','artist');
...
$this->db->select('sum(extrahour * overtimerate) as total'); 
$this->db->from('peopleattdance');
$this->db->join('actualschedual', 'actualschedual.id = peopleattdance.aid');
//$this->db->join('lis_artist', 'lis_artist.pid = peopleattdance.peopleid');
//$this->db->where('lis_artist.type','production');
...
//if($query8->num_rows()>0){
	...
	$sql ="select sum(extrahour * overtimerate) as total
	...
	where actualschedual.id = peopleattdance.aid
	  ...
	  and actualschedual.date <= '$todate'" . ' ' . $productionovertimecost;
	$file = fopen("ravitest4.txt","w");
	fwrite($file,$sql);
	fclose($file);	  
...
$query10 = $this->db->get();
	if($query10->num_rows() > 0){
		foreach($query10->result_array() as $listprodata){
			...
			//echo $unit;
			if($unit==1){
				$this->db->select('sum(extrahour)as total,count(actualschedual.id) as daycount,overtimerate,perdayrate'); 
				$this->db->from('peopleattdance');
				$this->db->join('actualschedual', 'actualschedual.id = peopleattdance.aid');
				$this->db->where('peopleattdance.peopletypeid','2');
				$this->db->where("actualschedual.projectid",$project);
			...
			//	$actualproductioncost=$productiontotal['total'];
						...
						$overtimerate=$productiontotal['overtimerate'];
						$rate=$productiontotal['perdayrate'];
						$overtimeamt=0;
			...
			//	$rate=round($rate/$dayinmmonthnumber);
				$this->db->select('sum(extrahour)as total,count(actualschedual.id) as daycount,perdayrate,overtimerate'); 
				$this->db->from('peopleattdance');
				$this->db->join('actualschedual', 'actualschedual.id = peopleattdance.aid');
				$this->db->where('peopleattdance.peopletypeid','2');
				$this->db->where("actualschedual.projectid",$project);
...
//PER DAY
	$this->db->select(' distinct(actualschedual.date) as ravidate, (peopleattdance.perdayrate + ( peopleattdance.overtimerate * peopleattdance.extrahour)) as amount, peopleattdance.extrahour, peopleattdance.*, character_master.name as charecter, people_master.name as peoplename, peopletype_master.name as peopletype, people_master.official_working_hour, actualschedual.unitno, actualschedual.location, actualschedual.date as rdate, peopleattdance.invoice_no ');
	$this->db->from('peopleattdance');
	$this->db->join('actualschedual', 'actualschedual.id 		= peopleattdance.aid');
	$this->db->join('peopletype_master', 'peopletype_master.id 	= peopleattdance.peopletypeid');
	$this->db->join('character_master', 'character_master.id 	= peopleattdance.charecterid');
	$this->db->join('people_master', 'people_master.id 			= peopleattdance.peopleid');
	$this->db->join('lis_artist', 'lis_artist.pid 				= peopleattdance.peopleid'); //RAVI
	//$this->db->join('people_master', 'people_master.id 			= lis_artist.pid'); //RAVI perunit
	$this->db->where('lis_artist.perunit', 1); //RAVI per day
	$this->db->where('actualschedual.date >=', $fromdate);
	$this->db->where('actualschedual.date <=', $todate);
	$this->db->where('actualschedual.projectid', $project);
	$this->db->where('peopleattdance.peopletypeid', 2);
	...
	$actualproduction =0 ;
foreach($query8->result_array() as $daata){
	$actualproduction= $actualproduction +  $daata['amount'];
}


//PER Month 
	$actualproduction_d=0;
	//$this->db->group_by('actualschedual.date');
	
	//$this->db->select('lis_artist.*,character_master.name as charectername,people_master.name as peoplename');    
	$this->db->select('sum(rate) as amount');    
	$this->db->from('lis_artist');
	$this->db->join('character_master', 'character_master.id = lis_artist.role_id');
	$this->db->join('people_master', 'people_master.id = lis_artist.pid');
	$this->db->where('lis_artist.status', 1);
	$this->db->where('lis_artist.perunit', 2);
	$this->db->where('lis_artist.proid', $project);
				
	$query8 = $this->db->get();
foreach($query8->result_array() as $daata){
	$actualproduction_d=$daata['amount'];
}
...
//RAVI 15-oct  Actual Expense
$this->db->select('sum(actualexpense.rate * actualexpense.qty) as total_exp'); 
	$this->db->from('actualexpense');
	$this->db->join('expenses_subaccount', 'expenses_subaccount.id = actualexpense.subaccountid ');
...
$query7 = $this->db->get();
foreach($query7->result_array() as $expensetotal){
	$actual_total_exp=$expensetotal['total_exp'] ;
}

//$actual_total_exp = $actual_total_exp + $productionovertimecost;
$actual_total_exp = $actual_total_exp ;
...
'actalproduction_d'=>$actualproduction_d,
...
function getexpensereportsdata($project,$fromdate,$todate){
    $result=array();
    $this->db->select('actualexpense.qty as qty, actualexpense.rate as rate, actualexpense.plannedamt, actualexpense.expenseid, 
    actualexpense.narration as narration, actualexpense.paymentmode as paymentmode, expenses_master.name as expenname, expenses_master.unit, unitofmeasurement.name as unitname, expenses_subaccount.name as subaccname, actualschedual.date as date');
	$this->db->from('actualexpense');
	$this->db->join('expenses_subaccount', 'expenses_subaccount.id = actualexpense.subaccountid');
	$this->db->join('expenses_master', 'expenses_master.id = actualexpense.expenseid');
	$this->db->join('unitofmeasurement', 'unitofmeasurement.id = expenses_master.unit');
...
/*	$sql = "select actualexpense.qty as qty, actualexpense.rate as rate, actualexpense.plannedamt, actualexpense.expenseid, expenses_master.name as expenname,expenses_master.unit,unitofmeasurement.name as unitname,expenses_subaccount.name as subaccname, actualschedual.date
	from  actualexpense, expenses_subaccount, expenses_master, unitofmeasurement, actualschedual
	where actualschedual.id = actualexpense.aid
	and expenses_master.id = actualexpense.expenseid
	and unitofmeasurement.id = expenses_master.unit
	and actualschedual.date >='$fromdate'
	and actualschedual.date <='$todate'
	and actualschedual.projectid ='$project'
	and expenses_subaccount.id = actualexpense.subaccountid
	order by actualschedual.date desc ";
...
/* RAVI
$this->db->select('(peopleattdance.perdayrate + ( peopleattdance.overtimerate * peopleattdance.extrahour)) as amount, peopleattdance.extrahour, peopleattdance.*, 
character_master.name as charecter, people_master.name as peoplename, peopletype_master.name as peopletype, people_master.official_working_hour, actualschedual.unitno, 
actualschedual.location, actualschedual.date as rdate, peopleattdance.invoice_no ');
	$this->db->from('peopleattdance');
	$this->db->join('actualschedual', 'actualschedual.id 		= peopleattdance.aid');
...
RAVI */
	...
	$this->db->select("( IF(peopleattdance.peopletypeid='2',round(peopleattdance.perdayrate / DAY(LAST_DAY(actualschedual.date)), 2), peopleattdance.perdayrate ) + ( peopleattdance.overtimerate * peopleattdance.extrahour)) as amount, peopleattdance.extrahour, 
	character_master.name as charecter, people_master.name as peoplename, unitofmeasurement.name as perunit, peopletype_master.name as peopletype, 
	people_master.official_working_hour, actualschedual.unitno, actualschedual.location, actualschedual.date as rdate, 
	 peopleattdance.perdayrate as perdayrate, 
	peopleattdance.overtimerate, peopleattdance.extrahour, peopleattdance.duration, peopleattdance.actualtotime, peopleattdance.actualfromtime, 
	peopleattdance.invoice_no, peopleattdance.peopletypeid, peopleattdance.charecterid	");
	...
	$this->db->join('people_master', 'people_master.id 			= peopleattdance.peopleid');
	$this->db->join('lis_artist', 'lis_artist.pid 				= peopleattdance.peopleid');
	$this->db->join('unitofmeasurement', 'lis_artist.perunit 	= unitofmeasurement.id');
...
/*$sql ="select (peopleattdance.perdayrate + ( peopleattdance.overtimerate * peopleattdance.extrahour)) as amount, peopleattdance.extrahour, peopleattdance.*, 
character_master.name as charecter, people_master.name as peoplename, peopletype_master.name as peopletype,  unitofmeasurement.name as perunit, 
people_master.official_working_hour, actualschedual.unitno, 
actualschedual.location, actualschedual.date as rdate, peopleattdance.invoice_no
from peopleattdance, actualschedual, peopletype_master, character_master, people_master, lis_Artist, unitofmeasurement
where actualschedual.id 		= peopleattdance.aid
  ...
  and character_master.id 	= peopleattdance.charecterid
	...
	and lis_artist.perunit 	= unitofmeasurement.id
	and actualschedual.date >='$fromdate'
	and actualschedual.date <='$todate'
	and actualschedual.projectid ='$project'
...
*/
	...
	function getpeolperate($project,$people,$date){
		...
		if($hasil1->num_rows()>0){
			...
			if($perunit==2){
												if($perdayrate >0){
													$perdayrate=$perdayrate / $dayinmmonthnumber;
												}else{
													$perdayrate=0;
												}
...
function getextrahourproductionstaff($project,$fromdate,$todate,$fromyear,$frommonth,$toyear,$tomonth){
	$result=array();
	...
	if($hasil1->num_rows()>0){
		foreach($hasil1->result_array() as $list_artistdata){
			...
			$actalrate=0;
//	$this->db->distinct('actualschedual.date');
	$this->db->select('*,actualschedual.date'); 
	$this->db->from('peopleattdance');
	$this->db->join('actualschedual', 'actualschedual.id = peopleattdance.aid');
	$this->db->where("actualschedual.projectid",$project);
	$this->db->where("peopleattdance.duration !=",'0:0:0');
...
public function getallfotage_detalis($project,$fromdate,$todate,$fromyear,$frommonth,$toyear,$tomonth){
	$result=array();
...
$Date = getDatesFromRange($fromdate, $todate); 
	foreach($Date as $date) {  
		...
		$difference=0;  
		$getsec=0; 
		$actualfotagedata=array();  
		$targetedfottage=array();  
		
		
	
		      
		$this->db->select('actualfotage,actualschedual.unitno,planfotage'); 
		$this->db->from('actualscenedetails');
		$this->db->join('actualschedual', 'actualschedual.id = actualscenedetails.aid');
		$this->db->where('actualschedual.date', $date);
		$this->db->where("actualschedual.projectid",$project);
		$this->db->where("actualscenedetails.actualfotage !=",'');
		$query2 = $this->db->get();
	foreach($query2->result_array() as $res4){
			...
			$unitno=$res4['unitno'];
			$actualfotage=$res4['actualfotage'];
			$planfotage=$res4['planfotage'];
			$datedate=$date;
			$getsec=$sec1;
...
public function getallepisodicstaffrate($project){
	$this->db->select('*,people_master.name as peoplename,character_master.name as Designation');    
	$this->db->from('lis_artist');
	$this->db->join('people_master', 'people_master.id = lis_artist.pid');
	$this->db->join('character_master', 'character_master.id = lis_artist.role_id');
	$this->db->where('lis_artist.proid', $project);
...
//PER MONTH Details
public function getallactalprodpmrate($project){
...
//PER DAY Details
public function getallactalprodpdayrate($project,$fromdate,$todate){
	
	$this->db->select(' distinct((peopleattdance.perdayrate + ( peopleattdance.overtimerate * peopleattdance.extrahour))) as amount, peopleattdance.extrahour, peopleattdance.*, character_master.name as charecter, people_master.name as peoplename, peopletype_master.name as peopletype, people_master.official_working_hour, actualschedual.unitno, actualschedual.location, actualschedual.date as rdate, peopleattdance.invoice_no ');
	$this->db->from('peopleattdance');
	$this->db->join('actualschedual', 'actualschedual.id 		= peopleattdance.aid');
	$this->db->join('peopletype_master', 'peopletype_master.id 	= peopleattdance.peopletypeid');
	$this->db->join('character_master', 'character_master.id 	= peopleattdance.charecterid');
...
//RAVI 15-oct  Actual Expense //"AAA" as name, " BBB" as code,
public function getactualexp($project,$fromdate,$todate){
	$this->db->select('actualschedual.date, expenses_master.name, expenses_master.code, actualexpense.expenseid, actualexpense.narration, actualexpense.rate, actualexpense.qty, round(actualexpense.rate * actualexpense.qty,2) as amount, actualexpense.subaccountid '); 
	$this->db->from('actualexpense');
	$this->db->join('expenses_subaccount', 'expenses_subaccount.id = actualexpense.subaccountid ');
	$this->db->join('actualschedual', 'actualschedual.id = actualexpense.aid');
	$this->db->join('expenses_master', 'expenses_master.id = actualexpense.expenseid');				
