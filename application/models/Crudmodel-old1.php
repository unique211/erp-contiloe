...
if($status == 1)
{
		$this->db->select('sum(rate) as plannedamt, (expensesbudget.rate*expensesbudget.qty) as plannedamt_mth, expenses_master.unit as perunit, expensesbudget.unit as unitnum');
		$this->db->from('expensesbudget');
		$this->db->join('expenses_master', 'expenses_master.id = expensesbudget.expensesid');
		$this->db->join('monthy_budget', 'monthy_budget.id = expensesbudget.budid');
		$this->db->where_in('month',$month);
		...
		$this->db->order_by('expenses_master.name');
			...
			$this->db->select('sum(rate) as plannedamt, (expensesbudget.rate*expensesbudget.qty) as plannedamt_mth, expenses_master.unit as perunit, expensesbudget.unit as unitnum,expenses_master.name as expences_names');
		$this->db->from('expensesbudget');
		$this->db->join('expenses_master', 'expenses_master.id = expensesbudget.expensesid');
		$this->db->join('monthy_budget', 'monthy_budget.id = expensesbudget.budid');
		$this->db->where_in('month',$month);
...
function getexpensesubaccount1($id,$fromdate,$todate,$project,$status){
