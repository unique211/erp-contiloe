...
//24June 
			...
			$data = array(
				...
				'effecitivedate' => $this->input->post('effecitivedatefrom'),
				'leavingdate' => $this->input->post('leavingdateto'),
				'status' => $this->input->post('status'),
				'attachment' => $this->input->post('filename'),
...
//RAVI
	...
	public function valid_trans_artist(){
		
		$table_name	= $this->input->post('table_name');
		$where	= $this->input->post('where');
		$pid	= $this->input->post('pid');
		
//SELECT count(*) as cnt FROM `callsheet` a , lis_artist b where a.peopleid = b.pid and a.peopleid = 107
		
		$this->db->select('count(*) as cnt');    
		$this->db->from('callsheet');
		$this->db->join( 'lis_artist', 'callsheet.peopleid = lis_artist.pid' );
		$this->db->where('callsheet.peopleid', $pid);
		$query = $this->db->get();
		$sql = $this->db->last_query();
		
//		$sql .=  ' <<TABLE NAME>> ' . $table_name . ' <<WHERE>> ' . $where ;
//		//echo $sql . ' ' . $table_name . ' ' ;
...
public function get_budget_vs_actual(){
	...
	public function get_payment_req_report()
	{
		 ...
		 $select_payment=$this->db->query("SELECT a.id as invoice_id, a.*, f.name as type_name,e.name as character_name,(grossamt-totalpayment) as final_amt,b.name as sup_name,c.companyname as comp_name FROM `invoice_entry` a left join people_master b on a.supplierid=b.id left join suppiler_master c on a.supplierid=c.id left join lis_artist d on a.supplierid=d.pid left join character_master e on d.role_id=e.id left join peopletype_master f on b.ptypeid=f.id where due_date between '$fromdate' and '$todate' and a.projectid='$project' order by due_date desc ");

		  foreach($select_payment->result() as $select_payment_data)
       {
           $invoice_id=$select_payment_data->invoice_id;
           $invoiceno=$select_payment_data->invoiceno;
           $invoice_date=date('d-m-Y', strtotime($select_payment_data->invoice_date));
           $due_date=date('d-m-Y', strtotime($select_payment_data->due_date));
           ...
           $type_name=$select_payment_data->type_name;
           $explode_sup_date=explode('-',$suppli_invoice_date);
           $new_sup_invoice_date=$explode_sup_date[1].'-'.$explode_sup_date[0];


            $totalpayment=0;
            $select_paidamt=$this->db->query("select invoiceid, paymentid, paidamt, invoice_amt from payment_detalis  where invoiceid = '$invoice_id' ");
			foreach($select_paidamt->result() as $select_paid_data){
				$totalpayment = $totalpayment + $select_paid_data->paidamt;
				
			    //$sql = $totalpayment + $select_paid_data->paidamt;
			}
            
            $sql = "select invoiceid, paymentid, paidamt, invoice_amt from payment_detalis  where invoiceid = '$invoice_id' ";
            $file = fopen("ravitest1.txt","a");
            fwrite($file,$sql);
            fclose($file);
           ...
           else
           {
            ...
            $balance_amt = $invoice_amt - $totalpayment;
            
           ?>
 <!-- round(SUM(a.t_sqft),0) -->
 			<td><?= $type_name?></td>
           	<td><?= $character_name ?></td>
           	<td style="text-align: right"><?= $invoice_amt ?></td>
           	<td style="text-align: right"><?= round(($totalpayment),0) ?></td>
           	<td style="text-align: right"><?= round(($balance_amt),0) ?></td>
	...
	//updates by mohammed
		public function get_payment_req_report1()
	{
		 $fromdate=$this->input->post('fromdate');
		 $todate=$this->input->post('todate');
		 $project=$this->input->post('project');
	...
	public function updaterate_func(){
		
		 $update_rate = $this->db->query("update actualschedual a, peopleattdance  b, `lis_artist` c 
							set b.perdayrate = c.rate, b.overtimerate = c.overtime 
							where a.id = b.aid 
								and c.pid = b.peopleid 
								and c.role_id = b.charecterid 
								and a.date >= c.effecitivedate and a.date <= c.leavingdate");

	}
	
	
	public function getactalprodpminfo(){
	...
	//	$project='1';
		$data=$this->Crudmodel->getallactalprodpmrate($project);
		echo json_encode($data);
	...
	public function getactalprodpdinfo(){
	...
	//	$project='1';
		$data=$this->Crudmodel->getallactalprodpdayrate($project,$fromdate,$todate);
		echo json_encode($data);
	...
	public function getactualexpbrakup(){
	...
	//	$project='1';
		$data=$this->Crudmodel->getactualexp($project,$fromdate,$todate);
		echo json_encode($data);
