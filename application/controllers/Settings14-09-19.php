...
public function get_budget_vs_actual(){
		...
		public function get_payment_req_report()
	{
        ...
        // $sumbalance_amt='';
		 ...
		 $select_payment=$this->db->query("SELECT *,e.name as character_name,(grossamt-totalpayment) as final_amt,b.name as sup_name,c.companyname as comp_name FROM `invoice_entry` a left join people_master b on a.supplierid=b.id left join suppiler_master c on a.supplierid=c.id left join lis_artist d on a.supplierid=d.pid left join character_master e on d.role_id=e.id where due_date between '$fromdate' and '$todate' and a.projectid='$project' order by due_date");
