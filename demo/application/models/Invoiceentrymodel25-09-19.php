<?php
class Invoiceentrymodel extends CI_Model{
    function get_insertid($table,$id){
        // $this->db->select_max($id);
        // $hasil=$this->db->get($table);
        // return $hasil->row()->$id;

        $query="SHOW TABLE STATUS LIKE 'invoice_entry'";
...
function getallinvoicedata(){
   ...
   if($hasil->num_rows() >0){
        foreach($hasil->result_array() as $invoicedata){
            ...
            $total_gross_amt=$grossamt+$sgst+$cgst+$igst;
            ...
            $result[]=array(
               ...
               'projectid'=>$projectid,
                ...
                'expens_id'=>$expens_id,
                'grossamt'=>$grossamt,
                'total_gross_amt'=>$total_gross_amt,
...
function getpaymentinfo($id){
    ...
    $hasil1=$this->db->get(); 
    if($hasil1->num_rows()>0){
        foreach($hasil1->result_array() as $paymentdata){
            $paidamt=$paymentdata['paidamt'];
            ...
            if($paymentid >0){
                ...
                if($hasil2->num_rows()>0){
                    foreach($hasil2->result_array() as $paymententrydaya){
                        $payment_date=$paymententrydaya['payment_date'];
                        $paid_via=$paymententrydaya['paid_via'];
                        $payment_ref=$paymententrydaya['payment_ref'];
                        $deductoinamt1=$paymententrydaya['deductoinamt1'];
                        $deductoinamt2=$paymententrydaya['deductoinamt2'];
                        $deductiomamt3=$paymententrydaya['deductiomamt3'];
...
function getplannfotage_data($project,$month,$year){
    $this->db->select('totalepisode');    
...
function getpeoleworkinhour($peopelid){
    $this->db->select('*');    
    $this->db->from('people_master');
    $this->db->where('id',$peopelid);
    $hasil=$this->db->get();
    return $hasil->result();
}
function check_invoicepayment($id){
    $this->db->select('*');    
    $this->db->from('payment_detalis');
    $this->db->where('invoiceid',$id);
    $hasil=$this->db->get();
...
function delete_invoice($id,$relationtable,$table){
    $result ="";
    if($relationtable=="actualexpense"){
        $data = array(
            'invoice_no' =>0,
            'invoice_date' =>'0000-00-00',
            'balance_amt' =>0,
        ...
        $this->db->where('invoice_no',$id);
         $this->db->update('actualexpense',$data);
    }elseif($relationtable=="peopleattdance"){
        $data = array(
            'invoice_no' =>0,
            'perdayrate' =>0,
            'passedamt' =>0,
         ...
         $this->db->where('invoice_no',$id);
         $this->db->update('peopleattdance',$data);
    }
    if($table=="invoice_entry"){
     /*--------Delete For invoiceattachment table------------*/ 
        $this->db->where('invoiceid',$id);
        $this->db->delete('invoiceattachment');
      
        /*--------Delete For invoice_entry  table------------*/ 
        $data = array(
            'status' =>0,
