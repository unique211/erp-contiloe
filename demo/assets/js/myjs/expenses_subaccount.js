$(document).ready(function() {
	var table_name = "expenses_subaccount";
    //alert('hi');
/*---------insert data into area_master start-----------------*/
    $(document).on("submit","#expensessubaccount",function(e){
			e.preventDefault();
         
			var name = $('#name').val();
            var expenseid = $('#expensesaccount').val();
...
function form_clear(){
    ...
    $('#save_update').val('');
    $('#name').val('');
    $('#expensesaccount').val('').trigger('change');
...
$(document).on('click','.edit_data',function(){
    ...
    var expenseid=$('#expensesid_'+id1).html();
       ...
       /* $('#date').val(date);
        ...
        $('#save_update').val(id1);
         ...
         $('#name').val(name);
         $('#expensesaccount').val(expenseid).trigger('change');
       // $('#date').val();
...
function show_master(){
  var where="expenses_subaccount.status='1'";
    $.ajax({
        type  : 'POST',
        ...
        var html = '';
            html += '<table id="myTable" class="table table-striped">'+
                    ...
                    '<th style="white-space:nowrap;text-align:left;padding:10px 10px;display:none;">Expenses id</th>'+
                    '<th style="white-space:nowrap;text-align:left;padding:10px 10px;">Expenses Account</th>'+
        ...
        for(i=0; i<data.length; i++){
           ...
           /* if(data[i].status==1){
            ...
            html += '<tr>'+
                    ...
                    '<td style="display:none;" id="expensesid_'+data[i].id+'">'+data[i].expensesid+'</td>'+
                    '<td id="expensesname_'+data[i].id+'">'+data[i].expensesname+'</td>'+
