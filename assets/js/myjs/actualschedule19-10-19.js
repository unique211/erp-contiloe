$(document).ready(function() {
    ...
    var date3 = $("#date").val();
    var tdateAr = date3.split('/');
    var day = getDaysInMonth(tdateAr[1], tdateAr[2]);
    var tblfromdate = '1' + '/' + tdateAr[1] + '/' + tdateAr[2];
    var tbltodate = day + '/' + tdateAr[1] + '/' + tdateAr[2];

    $('#tblfromdate').val(tblfromdate);
    $('#tbltodate').val(tbltodate);
    var tblfromdate1 = tdateAr[2] + '-' + tdateAr[1] + '-' + '1';
    var tbltodate1 = tdateAr[2] + '-' + tdateAr[1] + '-' + day;

    show_master(tblfromdate1, tbltodate1);

    $('#wait').hide();
    $('#wait1').hide();
    $('#copyactualschedual').hide();
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.length > 0) {
                                ...
                                console.log("scenewisedetails" + data);
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.length > 0) {
                                ...
                                console.log("callsheet" + data);
                                var row_id = 0;
                                $('#Scheduled_from').val(data[0].fromshift);
                                $('#Scheduled_to').val(data[0].toshift);
                                for (var i = 0; i < data.length; i++) {
                                    row_id = row_id + 1;


                                    var html = '<tr>' +
                                        '<td style="display:none;"><lable id="officialworkinhour_' + row_id + '" >' + data[i].official_working_hour + '</lable></td>' +
                                        ...
                                        '<td id="fromshifttimedata_"><div class="input-group timepicker" > <input type="text" class="form-control actualtime" id="actualshiftfromtime_' + row_id + '" size="20%" placeholder="hh:mm:ss"> <span class="input-group-addon"> <span class="glyphicon glyphicon-calendar"></span> </span></div></td>' +
                                        ...
                                        '<td style="display:none;"  id="perdayrate_' + row_id + '">' + 0 + '</td>' +
                                        '<td style="display:none;"   id="overtimerate_' + row_id + '">' + 0 + '</td>' +
                                        // '<td style="display:none;" id="officialworkinhour_' + row_id + '" >' + data[i].official_working_hour + '</td>' +
    ...
    $(document).on("submit", "#peopleattend_form", function(e) {
        ...
        if (save_update != "") {
            ...
            $("#perdayrate_" + save_update).html(0);
            $("#overtimerate_" + save_update).html(0);
            ...
            var html = '<tr class="project_tab_add_row" id="' + row_id + '" >' +
                ...
                '<td width="10%" id="peopletype_' + row_id + '">' + peopletype + '</td>' +
                '<td style="display:none;" id="charecter_' + row_id + '">' + character + '</td>' +
                '<td size="10%"  id="charectername_' + row_id + '">' + charactername + '</td>' +
                ...
                '<td id="fromshifttimedata_"><div class="input-group timepicker" > <input type="text" class="form-control actualtime" id="actualshiftfromtime_' + row_id + '" size="20%" placeholder="hh:mm:ss" value=' + actualfrom + '> <span class="input-group-addon"> <span class="glyphicon glyphicon-calendar"></span> </span></div></td>' +
                '<td id="fromshiftotimedata_"><div class="input-group timepicker" > <input type="text" class="form-control actualtime" id="actualshifttotime_' + row_id + '" size="20%" placeholder="hh:mm:ss" value=' + actualto + '> <span class="input-group-addon"> <span class="glyphicon glyphicon-calendar"></span> </span></div></td>' +
                '<td id="duration_"><div class="input-group timepicker" > <input type="text" class="form-control" id="actalduration_' + row_id + '"  size="20%" placeholder="hh:mm:ss" value=' + durationdata + '> <span class="input-group-addon"> <span class="glyphicon glyphicon-calendar"></span> </span></div></td>' +
                '<td  id="extrahour_' + row_id + '"><input type="text" id="extratime_' + row_id + '" size="5%" value=' + extratime + ' ></td>' +
                '<td  id="callremark"><textarea  id="actualremarkdata_' + row_id + '"   cols="13" value=' + remarks + '>' + remarks + '</textarea></td>' +
                '<td style="display:none;"  id="perdayrate_' + row_id + '">' + 0 + '</td>' +
                '<td style="display:none;"   id="overtimerate_' + row_id + '">' + 0 + '</td>' +
                '<td><button  class="doc_edit_data1 btn btn-xs btn-primary"   id="' + row_id + '"  ><i class="fa fa-edit"></i></button>&nbsp;&nbsp;<button  class="regional_delete_data1 btn btn-xs btn-danger"   id="' + row_id + '"  ><i class="fa fa-trash"></i></button>' +
                // '<td id="getused_"><select   class="form-control input-sm unit" id="unit" name="unit" width="10%"><option selected disabled value="" width="20%">Select</option> </select></td>'+
        ...
        //$('#people_type').val('').trigger('change');
        $('.roledata').val('').trigger('change');
        $('.peopledata').val('').trigger('change');
        $('#Scheduled_from').val('');
        $('#Scheduled_to').val('');
        $('#Actual_from').val('');
        $('#actual_to').val('');
    ...
    $(document).on("blur", "#actual_to", function(e) {
        ...
        var offcialworkinghour = $('#workinghour').val();

        // alert("offcialworkinghour" + offcialworkinghour);
        ...
        offcialworkinghour = offcialworkinghour.split(":");

        var durationhour = different[0] + "." + different[1];
        var officialdata = offcialworkinghour[0] + "." + offcialworkinghour[1];
        var extra_hour = 0;
        if (officialdata > 0) {

            extra_hour = parseFloat(durationhour) - parseFloat(officialdata);
            extra_hour = extra_hour.toFixed(2);
            console.log("extra_hour" + extra_hour);
            if (extra_hour > 0) {
                console.log('from here');
                extra_hour = extra_hour.split(".");
                if (extra_hour[1] > 0) {
                    extra_hour[1] = 5;
                } else {
                    extra_hour[1] = 0;
                }
                console.log("extra_hour" + extra_hour[0] + "" + extra_hour[1]);
                $('#extra_hour').val(extra_hour[0] + "." + extra_hour[1]);
            } else {
                $('#extra_hour').val('0');
            }
        }
    ...
    $(document).on('click', '.doc_edit_data1', function(e) {
        ...
        var actaltime = $("#actualshiftfromtime_" + row).val();
        var actualto = $("#actualshifttotime_" + row).val();
        var extratime = $("#extratime_" + row).val();
        var duration = $("#actalduration_" + row).val();
        var remarks = $("#actualremarkdata_" + row).val();
    ...
    $(document).on("submit", "#actualexpe_form", function(e) {
        ...
        if (save_update != "") {
            ...
            $("#expensive_data" + save_update).html(expensivetype);
            ...
            var markup = '<tr  id="sr_' + sr + '">' +
                ...
                '<td  width="15%;"  id="narro_"><input type="text" id="narroation_' + sr + '"   value=" ' + narroation + '"></td>' +
                ...
                '<td><button  class="doc_edit_data2 btn btn-xs btn-primary"   id="' + sr + '"  ><i class="fa fa-edit"></i></button>&nbsp;&nbsp;<button  class="regional_delete_data2 btn btn-xs btn-danger"   id="sr_' + sr + '"  ><i class="fa fa-trash"></i></button>' +
                '</tr>';
    ...
    $(document).on('click', '.doc_edit_data2', function(e) {
        ...
        $('#expense_sub_ac').val(expensive_sub_ac).trigger('change');

        $('#suppliername').val(supplier).trigger('change');
    ...
    $(document).on("click", "#peoplesave", function(e) {
        ...
        for (var j = 0; j < r; j++) {
            ...
            var perdayrate = $(l1[j]).find('td:eq(14)').html();
            var overtimerate = $(l1[j]).find('td:eq(15)').html();

            console.log("perdayrate" + perdayrate + "overtimerate" + overtimerate);
            if (perdayrate == 0 && overtimerate == 0) {
                console.log('from perday');
                $.ajax({
                    type: "POST",
                    url: baseurl + "settings/getpeoplerate",
                    dataType: "JSON",
                    async: false,
                    data: {
                        peopleid: peopleid,
                    ...
                    success: function(data) {
                        ...
                        console.log("perdayrate" + perdayrate + "overtimerate" + overtimerate);
                        if (data.length > 0) {
                            perdayrate1 = data[0].perdayrate;
                            overtimerate1 = data[0].overtimerate;
                        }
                        perdayrate1 = (parseFloat(perdayrate1).toFixed(2));
                        overtimerate1 = (parseFloat(overtimerate1).toFixed(2));
                        $.ajax({
                            type: "POST",
                            url: baseurl + "settings/save_settings",
                            dataType: "JSON",
    ...
    function show_master(fromdate, todate) {
        ...
        $.ajax({
            type: "POST",
            url: baseurl + "settings/gealltactualdata",
            dataType: "JSON",
            async: false,
            data: {
                fromdate: fromdate,
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.length > 0) {
                    ...
                    $('#Scheduled_from').val(data[0].fromshifttime);
                    $('#Scheduled_to').val(data[0].toshifttime);
                    for (i = 0; i < data.length; i++) {
                        row_id = row_id + 1;
                        ...
                        var html = '<tr>' +
                            ...
                            '<td id="fromshifttimedata_"><div class="input-group timepicker" > <input type="text" class="form-control actualtime" id="actualshiftfromtime_' + row_id + '" size="20%" placeholder="hh:mm:ss" value=' + data[i].actualfromtime + '> <span class="input-group-addon"> <span class="glyphicon glyphicon-calendar"></span> </span></div></td>' +
                            ...
                            '<td style="display:none;"  id="perdayrate_' + row_id + '">' + data[i].perdayrate + '</td>' +
                            '<td style="display:none;"   id="overtimerate_' + row_id + '">' + data[i].overtimerate + '</td>' +
                            // '<td id="getused_"><select   class="form-control input-sm unit" id="unit" name="unit" width="10%"><option selected disabled value="" width="20%">Select</option> </select></td>'+
    ...
    function settimeincallfrom() {
        ...
        for (var i = 0; i < r; i++) {
            row_id = row_id + 1;
            var offcialworkinghour = $('#officialworkinhour_' + row_id).text();
            ...
            if (shifttimeto > shifttime) {
                ...
                offcialworkinghour = offcialworkinghour.split(":");
                console.log("offcialworkinghour" + offcialworkinghour[1]);
                var durationhour = hour + "." + min;
                var officialdata = offcialworkinghour[0] + "." + offcialworkinghour[1];
                ...
                if (officialdata > 0) {
                    ...
                    if (extra_hour > 0) {
                        ...
                        console.log("extra_hour" + extra_hour[0] + "" + extra_hour[1]);
                        $('#extratime_' + row_id).val(extra_hour[0] + "." + extra_hour[1]);
                    } else {
                        $('#extratime_' + row_id).val('0');
                    }
                } else {
                    if (min > 0) {
                        time = 5;
                    }
                    $('#extratime_' + row_id).val(hour + "." + time);
                }
    ...
    function getextrahour() {
        ...
        for (var i = 0; i < r; i++) {
            row_id = row_id + 1;
            var duration = $('#actalduration_' + row_id).val();
            var people = $('#people_' + row_id).val();
            if (people > 0) {
                $.ajax({
                    type: "POST",
                    url: baseurl + "settings/getextrahour",
                    dataType: "JSON",
                    data: {
                        id: id1,
    ...
    $(document).on("blur", ".actualtime", function(e) {
        ...
        var id = $(this).attr('id');
        id = id.split('_');
        var end = $('#actualshifttotime_' + id[1]).val();
        console.log("end date" + end);
        var start = $('#actualshiftfromtime_' + id[1]).val();
        var offcialworkinghour = $('#officialworkinhour_' + id[1]).text();
        if (end != "") {
            ...
            if (officialdata > 0) {
                ...
                if (extra_hour > 0) {
                    ...
                    console.log("extra_hour" + extra_hour[0] + "" + extra_hour[1]);
                    var extratime = extra_hour[0] + "." + extra_hour[1];
                    extratime = Math.ceil(extratime);
                    $('#extratime_' + id[1]).val(extratime);
                } else {
                    $('#extratime_' + id[1]).val('0');
                }
            } else {
                if (min > 0) {
                    time = 5;
                }
                var extratime = hour + "." + time;
                extratime = Math.ceil(extratime);
                $('#extratime_' + id[1]).val(extratime);
            }
    ...
    function addTimes() {
        ...
        for (var k = 0; k < l; k++) {
            ...
            if (planfotage == '' || planfotage == '0') {} else {
    ...
    $(document).on("submit", "#actualcopydata", function(e) {
        ...
        $('.closehideshow').trigger('click');
        $('#filter').trigger('click'); //call function show all product					

    });

    /*---------------change Event OF peoplet type Start--------------*/
    $(document).on("change", "#peopledata", function(e) {
        e.preventDefault();
        var people = $('#people').val();
        ...
        $.ajax({
            ...
            type: "POST",
            url: baseurl + "Invoice_Entry/getpeopleworkinhour",
            dataType: "JSON",
            async: false,
            data: {
                table_name: "people_master",
            ...
            success: function(result) {
                if (result.length > 0) {
                    $('#saveworkinhour').val(result[0].official_working_hour);
                }
            }
        });
    ...
    /*---------------Click event of Filter Button Strat--------------*/
    $(document).on("click", "#filter", function(e) {
        e.preventDefault();
        var fromdate = $('#tblfromdate').val();
        var tdateAr = fromdate.split('/');
        var fromdate = tdateAr[2] + '-' + tdateAr[1] + '-' + tdateAr[0];

        var todate = $('#tbltodate').val();
