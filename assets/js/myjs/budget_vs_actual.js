$(document).ready(function() {
    ...
    $(document).on("submit", "#showwisereport", function(e) {
        ...
        //RAVI fromdate = tdateAr[2] + '-' + tdateAr[1] + '-01';
        // alert(status+" status");
...
*/
                    ...
                    success: function(data) {
                        ...
                        if (data != "") {
                            ...
                            if (data[0].productionovertimecost >= 0) {
                                productionstaffexcost = data[0].productionovertimecost;
                                markup5 += '<tr>' +
                                    '<td  style="border-left:solid 0px;" ><button id="getproductionexh"  class="btn btn-xs btn-info getexhour" ><i class="fa fa-angle-double-down"></i></button></td>' +
                        ...
                        if (status == 0) {
                            ...
                            if (productionstaffexcost > 0) {
                                expensetotal_total = parseFloat(expensetotal_total) + parseFloat(productionstaffexcost);
                            }
    ...
    /*----for production staff Extra hour detalis----*/
    $(document).on("click", ".getexhour", function() {

        var id = $(this).attr('id');

        $('#gettableunit').text('Production Staff Extra Hour Cost');
        var project = $('#project').val();
        var frommonth = $('#frommonth').val();
        var fromyear = $('#fromyear').val();
        var title = 'Production Staff Extra Hour Cost';
        ...
        $.ajax({
            type: "POST",
            url: baseurl + "settings/getproductionstaffexhourcost",
            dataType: "JSON",
            async: false,
            data: {
                project: project,
            ...
            success: function(data) {
                ...
                if (data.length > 0) {
                    ...
                    html += '<table id="myTable" class="table table-striped">' +
                        ...
                        '<th style="white-space:nowrap;text-align:left;padding:10px 10px;">Production Staff Name</th>' +
                        '<th style="white-space:nowrap;text-align:left;padding:10px 10px;">Date</th>' +
                        '<th style="white-space:nowrap;text-align:center;padding:10px 10px;">Overtime Rate</th>' +
                        '<th style="white-space:nowrap;text-align:right;padding:10px 10px;">Over Time Hour</th>' +
                        '<th style="white-space:nowrap;text-align:right;padding:10px 10px;">Total Over Time Amt</th>' +
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        grandtotal = (parseFloat(grandtotal) + parseFloat(data[i].overtimeamount)).toFixed(2);
                        html += '<tr>' +

                            '<td id="peopelname_' + data[i].id + '"><b>' + data[i].peoplename + '</b></td>' +
                            '<td id="day_' + data[i].id + '">' + date + '</td>' +
                            '<td  style="text-align:right;" id="overtimerate_' + data[i].id + '">' + parseFloat(data[i].overtimerate).toFixed(2) + '</td>' +
                            '<td style="text-align:right;" id="overtime_' + data[i].id + '">' + data[i].overtime + '</td>' +
                            '<td style="text-align:right;" id="total_' + data[i].id + '">' + parseFloat(data[i].overtimeamount).toFixed(2) + '</td>' +
                            '</tr>';
                    }
