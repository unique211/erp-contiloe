$(document).ready(function() {
    var expensegroup = new Array();
    var unitofmeasurement = new Array();
    ...
    $(document).on("click", "#btninadd", function(e) {
        ...
        $('#totalleave').text('0');
        $('#payment_totalamt').text('0');
        $('#worktype').val('').trigger('change');
        $('#noofeposoic').val('0');
        $('#perepisidicrate').val('0');
        $('#balanceamt').text('0');
        getMasterSelect("expensesgroup", ".expensegroup", " status = '1'");
        getMasterSelect("episodic_worktype", ".worktype", " status = '1'");
        getMasterSelect("unitofmeasurement", ".unit", " status = '1'");
        // getMasterSelect("suppiler_master","#supplier"," status = '1'");
        getMasterSelect("project_master", ".project_name", " status = '1'");
        getMasterSelect("peopletype_master", ".people_type", " status = '1'");
        getentryno();
    });
    ...
    function getMasterSelect(table_name, selecter, where) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                for (i = 0; i < data.length; i++) {
                    var id = '';
                    if (table_name == "expensesgroup") {
                        console.log(data[i].id);
                        expensegroup.push({ name: data[i].name, id: data[i].id });
                        /* expensegroup['name']=data[i].name;	
                         expensegroup['id']=data[i].id;*/
                    } else if (table_name == "unitofmeasurement") {
                        unitofmeasurement.push({ name: data[i].name, id: data[i].id });

                    } else if (table_name == "suppiler_master") {
                        supplier.push({ name: data[i].companyname, id: data[i].id });

                    } else if (table_name == "project_master") {
                        projectdata.push({ name: data[i].name, id: data[i].id });
                    }
        ...
        //---- for geting dropdown value-------*/
        /*$.each(expensegroup, function (index, value) {
            console.log("name: "+value.name +" and index: "+ value.id);
    ...
    var amt=0;
        if(dmh >0 && rate >0){
            amt=parseFloat(rate)* parseFloat(dmh)
        }else{
            amt=0;
        }
       $("#amount").val(amt);
    ...
    /*----------------function of total of table-----------*/
    function get_total() {
        var l1 = $('table#list_artist_tb').find('tbody').find('tr');
        var s = l1.length;
        var hour = 0;
        var min = 0;
        var sec = 0;
    ...
    /*------------------Delete For  list of Artist Form -------*/
    $(document).on('click', '.regional_delete_data', function(e) {
        e.preventDefault();

        var save_update = $(this).attr('id');

        var save_update1 = save_update.split("_")
        if (save_update1[1] != "") {
            $("#list_" + save_update1[1]).remove();
    ...
    function getentryno() {
        $.ajax({
            type: "POST",
            url: baseurl + "Invoice_Entry/getmaxid",
            data: {
    ...
    /*-------Change Event Of Vendor type*------------*/
    $(document).on("change", "#people_type", function(e) {
        e.preventDefault();
        ...
        if (people_typename == "Artist" || people_typename == "Production" || people_typename == "Episodic" || people_typename == "Admin Staff") {
            if (people_typename == "Admin Staff") {
                ...
                if (people_typename == "Artist") {
                    atype = 'artist';
                } else if (people_typename == "Production") {
                    atype = 'production';
            ...
            $('#exoenseadjustment').hide();
            $('#productionsave').removeAttr("disabled");
            if (people_typename == "Artist" || people_typename == "Production") {

                if (people_typename == "Production") {
                    $('#paid_leaveinfo').show();
                    $('#paid_leaveinfotb').show();
                    $('#production_data').show();
                    $('.episodic').hide();
                    $(".eposoidreq").prop('required', false);
                } else {
                    $('#paid_leaveinfo').hide();
                    $('#paid_leaveinfotb').hide();
                    $('#production_data').hide();
                    $('.episodic').hide();
                    $(".eposoidreq").prop('required', false);
                }
                $('#productionartist').show();
                //  $('#paid_leaveinfo').hide();

            } else {
                if (people_typename == "Production" || people_typename == "Admin Staff") {
                    $('#paid_leaveinfo').show();
                    $('#productionartist').hide();
                    $('#paid_leaveinfotb').show();
                    $('#production_data').hide();
                    $('.episodic').hide();
                    $(".eposoidreq").prop('required', false);
        ...
        $.ajax({
            type: "POST",
            url: baseurl + "Invoice_Entry/getsupplier",
            data: {
                table_name: table_name,
                where: where,
                atype: atype,
    ...
    $(document).on("change", ".creditday", function(e) {
        e.preventDefault();
        form_clear();
        var people_typename = $("#people_type option:selected").text();
        var id = $("#supplier").val();
        ...
        $.ajax({
            type: "POST",
            url: baseurl + "Invoice_Entry/get_gst_pan",
            data: {
    ...
    function getcriditdate() {
        ...
        date2 = tdateAr[1] + '-' + tdateAr[0] + '-' + day;

        console.log(date2);

        if (people_typename == "Artist" || people_typename == "Production" || people_typename == "Episodic" || people_typename == "Admin Staff") {
            table_name = 'lis_artist';
            where = "proid=" + project + " And pid=" + supplier;
        } else {
            table_name = "suppiler_master";
            where = 'id=' + supplier;
            project = 1;
        ...
        if (project > 0 && supplier > 0) {
            $.ajax({
                type: "POST",
                url: baseurl + "Invoice_Entry/getcriditdate",
                data: {
                    table_name: table_name,
                    where: where,
                    Sdate: date2,
                ...
                success: function(data) {
                    ...
                    if (data != "") {
                        var tdateAr1 = data.split('-');
                        var adddate = tdateAr1[2] + '/' + tdateAr1[1] + '/' + tdateAr1[0];
                        $('#d_date').val(adddate);
                    } else {
                        var tdateAr1 = Sdate.split('-');
                        var adddate = tdateAr1[2] + '/' + tdateAr1[1] + '/' + tdateAr1[0];
    ...
    $(document).on('submit', '#invoice_form', function(e) {
        ...
        var remarkdata = $('#remarkdata').val();

        var worktype = $('#worktype').val();
        var noofeposoic = $('#noofeposoic').val();
        var perepisidicrate = $('#perepisidicrate').val();


        var tdateAr1 = e_date.split('/');
        e_date = tdateAr1[2] + '-' + tdateAr1[1] + '-' + tdateAr1[0];

        var tdateAr1 = s_date.split('/');
        s_date = tdateAr1[1] + '-' + tdateAr1[0];

        var tdateAr1 = d_date.split('/');
        d_date = tdateAr1[2] + '-' + tdateAr1[1] + '-' + tdateAr1[0];

        var tdateAr1 = af_date.split('/');
        invoicedate = tdateAr1[2] + '-' + tdateAr1[1] + '-' + tdateAr1[0];
        ...
        $.ajax({
            ...
            data: {
                ...
                other: '',
                worktype: worktype,
                noofeposoic: noofeposoic,
                remarkdata: remarkdata,
                perepisidicrate: perepisidicrate,
                remain_status: 'partical',
                table_name: 'invoice_entry',
            ...
            success: function(data) {
                ...
                if (data > 0 || data == true) {
                    ...
                    console.log(aid);
                    console.log("length of table" + s);
                    for (var j = 1; j <= s; j++) {

                        var peopleid = $('#actualid_' + j).text();
                        var perrate = $('#perrate_' + j).text();
                        var overtimerate = $('#extrahour_' + j).text();
                        var apassamt = $('#apassamt_' + j).val();
                        var abalance = $('#abalance_' + j).val();
                        var date = $('#date_' + j).text();
                        //console.log("length of table" + s);

                        for (var k = 0; k < chdate.length; k++) {
                        ...
                        if ((abalance == 0 && (apassamt > 0 || apassamt == 0)) || (abalance > 0 && apassamt > 0)) {
                            // flag = 1;

                            console.log("peopleid" + peopleid + "" + apassamt + "" + abalance + "" + invoiceid);
                    ...
                    if (people_typename == "Artist" || people_typename == "Production") {
                        if (flag == 0) {
                            $.ajax({
                                type: "POST",
                                url: baseurl + "Invoice_Entry/getinvoiceupdate",
                                data: {
        ...
        $('.invoiceentry').show();
        $('.panel-tab a[href="#attachment"]').tab('show');
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                html += '<table id="myTable" class="table table-striped">' +
                    ...
                    // '<th style="white-space:nowrap;text-align:left;padding:10px 10px;">For Month</th>' +
                    ...
                    '<th style="white-space:nowrap;text-align:left;padding:10px 10px;">People/Supplier Name</th>' +
                    '<th style="display:none;">Supplierid</th>' +
                    '<th style="white-space:nowrap;text-align:left;padding:10px 10px;">Due Date</th>' +
                    '<th style="white-space:nowrap;text-align:left;padding:10px 10px;text-align:right">Invoice amt.</th>' +
                ...
                for (i = 0; i < data.length; i++) {
                    ...
                    if (data[i].date != "0000-00-00") {
                        ...
                        invoicedate = tdateAr[2] + '/' + tdateAr[1] + '/' + tdateAr[0];

                    }
                    if (data[i].date != "0000-00-00") {

                        var tdateAr = data[i].due_date.split('-');
                        duedate = tdateAr[2] + '/' + tdateAr[1] + '/' + tdateAr[0];
                    ...
                    html += '<tr>' +
                        ...
                        '<td id="invoice_datedata_' + data[i].id + '">' + invoicedate + '</td>' +
                        // '<td id="date_' + data[i].id + '">' + date + '</td>' +
                        ...
                        '<td id="duedatedata_' + data[i].id + '">' + duedate + '</td>' +
                        '<td style="text-align:right" id="totalgrossamt_' + data[i].id + '">' + data[i].total_gross_amt + '</td>' +
    ...
    $(document).on('click', '.edit_data', function() {
        ...
        var due_date = $('#duedatedata_' + id1).html();
        ...
        var invoice_date = $('#invoice_datedata_' + id1).html();
                                ...
                                if (data1.length > 0) {
                                    ...
                                    for (var i = 0; i < data1.length; i++) {
                                        ...
                                        if (vendor_type == 1) {
                                            ...
                                            var adate = tdateAr1[2] + '/' + tdateAr1[1] + '/' + tdateAr1[0];
                                            var overtimerate = "";
                                            if (data1[i].extrahour == "" || data1[i].extrahour == 0) {
                                                overtimerate = 0;

                                            } else {

                                                overtimerate = data1[i].overtime;
                                            }
   ...
   $('#other').val(othercharge);*/
    ...
    $(document).on('click', '.delete_data', function() {
        ...
        var people_typename = $("#people_type").val();
        ...
        if (people_typename == "Artist" || people_typename == "Production") {
            ...
            relationtable = "peopleattdance";
        } else if (people_typename == "Supplier") {
            relationtable = "actualexpense";
        } else {
            relationtable = "";
        }
        // alert(relationtable);
        if (id1 != "") {
            swal({
                ...
                function() {
                    /*-------check invioce payment entry Exist or Not---------------*/
                    $.ajax({
                        type: "POST",
                        url: baseurl + "Invoice_Entry/checkinvoicepayment",
                        dataType: "JSON",
                        data: {
                            id: id1,
                        ...
                        success: function(data) {
                            if (data == '100') {
                                swal({
                                    ...
                                    text: "For Selected Invoice entry exist in Payment Entry....",
                                    type: "warning",
                                    showCancelButton: true,
                                    confirmButtonColor: "#DD6B55",
                                ...
                                $.ajax({
                                    type: "POST",
                                    url: baseurl + "Invoice_Entry/deleteinvoicedata",
                                    dataType: "JSON",
                                    data: {
                                        id: id1,
    ...
    /*------------for not allowed Special Charecter -----------------*/
    $('.grosamtdata').on('keypress', function(event) {
        var regex = new RegExp("^[0-9]+$");
        var key = String.fromCharCode(!event.charCode ? event.which : event.charCode);
        if (!regex.test(key)) {
            event.preventDefault();

            // $('#lblgrossamterror').show();
        } else {
            //$('#lblgrossamterror').hide();
        }
    });
